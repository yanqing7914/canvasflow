...
export function planEffects(state: AirportPickupTaskState, event: AirportPickupEvent, toolResults: Record<string, unknown>): PlannedEffect[] {
  ...
  if (event.type === 'flight.updated' && event.flight.status === 'landed' && state.message.autoNotifyAuthorized && !state.message.landingNoticeSent && state.message.status === 'idle') return [{ type: 'message.send', status: 'planned', tool: 'message.send' }]
