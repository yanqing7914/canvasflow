...
export function planEffects(state: AirportPickupTaskState, event: AirportPickupEvent, toolResults: Record<string, unknown>): PlannedEffect[] {
  ...
  if (event.type === 'user.confirmed-passengers-onboard' && state.phase === 'returning-home' && 'memory.get-preferences' in toolResults) return [{ type: 'vehicle.apply-cabin-profile', status: 'succeeded', tool: 'vehicle.apply-cabin-profile' }]
