...
describe('Warm Cabin surface wiring', () => {
  ...
  it('keeps route graphics blue and action graphics terracotta', () => {
    ...
    expect(stylesheet).toMatch(/\.ui-charge-brief__station-distance\s*\{[^}]*color:\s*var\(--route\)/)
    expect(stylesheet).toMatch(/\.ui-charge-brief__route-dot\s*\{[^}]*background:\s*var\(--route\)/)
    expect(stylesheet).toMatch(/\.ui-charge-brief__soc-track span\s*\{[^}]*linear-gradient\(90deg,\s*var\(--route\),\s*var\(--state-ink-positive\)/)
    ...
    expect(stylesheet).not.toMatch(/var\(--blue\)/)
  })
})
