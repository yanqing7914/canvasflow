...
function declarationBlock(selector: string): string {
  ...
  if (!match) throw new Error(`No declaration block for ${selector}`)
  return match[1]
}
...
describe('Warm Cabin action contrast', () => {
  ...
  it.each(['light', 'dark'] as const)('keeps disabled actions readable in %s mode', (theme) => {
    const block = themeBlock(theme)
    const disabled = declarationBlock('.ui-action--primary:disabled')
  ...
  it('does not apply the primary hover surface to a disabled UISpec action', () => {
    expect(stylesheet).toContain('.ui-action--primary:hover:not(:disabled)')
    expect(stylesheet).not.toMatch(/\.ui-action--primary:hover\s*[,{]/)
  })
})
...
describe('Warm Cabin surface wiring', () => {
  ...
  it('keeps route graphics blue and action graphics terracotta', () => {
    ...
    for (const selector of ['.ui-route-sketch__line', '.ui-route-sketch__marker', '.ui-route-map__line', '.ui-route-map__marker']) {
      expect(declarationBlock(selector)).toMatch(/(?:stroke|fill):\s*var\(--route\)/)
    }
  })

  it('lets the idle map and facts follow the active cabin theme', () => {
    for (const selector of ['.idle-cockpit', '.idle-map', '.idle-cockpit__facts', '.idle-cockpit__actions button']) {
      const block = declarationBlock(selector)
      expect(block).toMatch(/background(?:-color)?:[^;]*var\(--(?:cabin|surface|control)/)
    }
    expect(stylesheet).not.toMatch(/\.idle-(?:cockpit|map)\b[^}]*background:\s*#(?:211711|f1ebe3)/i)
  })
})
